...
public:
  ...
  : Node("hdl_localization", options)
  {
    ...
    ndt_num_thread = declare_parameter<int>("ndt_num_thread", 1);
...
private:
  pcl::Registration<PointT, PointT>::Ptr create_registration() {
    if(reg_method == "NDT_OMP") {
      ...
      ndt->setNumThreads(ndt_num_thread);
      RCLCPP_INFO_STREAM(get_logger(), "NDT Thread Number : " << ndt_num_thread);
      return ndt;
    } else if(reg_method.find("NDT_CUDA") != std::string::npos) {
      RCLCPP_INFO(get_logger(), "NDT_CUDA is selected");
      pcl::shared_ptr<fast_gicp::NDTCuda<PointT, PointT>> ndt(new fast_gicp::NDTCuda<PointT, PointT>);
      ndt->setResolution(ndt_resolution);
...
private:
  ...
  void points_callback(const sensor_msgs::msg::PointCloud2::ConstSharedPtr points_msg) {
    ...
    // transform pointcloud into odom_child_frame_id //문제없음
    std::string tfError;
    pcl::PointCloud<PointT>::Ptr cloud(new pcl::PointCloud<PointT>());
    if(tf_buffer->canTransform(odom_child_frame_id, pcl_cloud->header.frame_id, stamp, rclcpp::Duration(std::chrono::milliseconds(100)), &tfError)){
